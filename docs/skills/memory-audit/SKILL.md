...
你不需要扫描整棵记忆树。你的工作范围是**当前视野内的记忆**——boot 时加载的、对话中读取过的、diagnostic 报出来的。不在眼前的，这次不管。

视野会在整理过程中自然扩展：检查一条记忆时，你可能需要读取它引用的其他节点——读出来了就进了视野，那就管。但要守住边界：因为当前检查需要而读出来的，管；因为好奇心顺着链接无限递归下去的，不管。

**建议的起手：**
1. **`system://diagnostic/<domain>`**（首选）。它会直接报出两类已确诊的病灶：
...
2. **对话中触及过的记忆节点**。如果刚结束一轮对话，从那次对话中读过的记忆开始，趁上下文还热。
3. **`system://index/<domain>`**。扫一眼结构，挑出"看着就觉得不对劲"的节点。
